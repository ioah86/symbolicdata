...
sub transformedData {
  ...
  my $out=<<EOT;
 <sd:$type rdf:about="$newid.Homog" sd:createdAt="$createdAt" >
...
EOT
  ...
  # map { s|/|.|gs; $out.=addValue("rdfs:comment",$_); } (@$comments);

  return <<EOT;
$out
   <sd:createCommand>homogenizeIdeal(basis,"hv")</sd:createCommand>
   <rdfs:comment>Homogenized version of $ref</rdfs:comment>
 </sd:$type>
EOT
